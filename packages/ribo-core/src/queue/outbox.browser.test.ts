...
/**
 * The sessions collection at v0 — before `extractionUsage`. Spelled out rather
 * than derived from `sessionRxSchema` so the test seeds what actually shipped:
 * deriving it would move with the code and prove nothing.
 */
const SESSION_RX_SCHEMA_V0: RxJsonSchema<Record<string, unknown>> = {
  version: 0,
  primaryKey: "id",
  type: "object",
  properties: {
...
test("a v0 session survives the v1 bump that added extractionUsage", async () => {
  // Adding an OPTIONAL field still needs the version bump: RxDB validates the
  // declared schema against the one the collection was created with, so adding a
  // property at the same version is refused at `addCollections` — a database that
  // will not open, not a field that quietly does nothing. This test is what makes
  // that claim checkable; without it the bump is an assertion in a comment.
  const name = uniqueName();
  addRxPlugin(RxDBAttachmentsPlugin);
  const db0 = await createRxDatabase({
    name,
  ...
  await db0.addCollections({ sessions: { schema: SESSION_RX_SCHEMA_V0 } });
  await db0.collections.sessions!.insert({
    id: "s-v0",
    status: "awaiting-review",
    openedAt: "2026-07-23T10:00:00.000Z",
    closedAt: "2026-07-23T11:00:00.000Z",
    ctx: { jobId: "job-7" },
    ref: "job-7",
    attempts: 0,
    nextAttemptAt: "2026-07-23T10:00:00.000Z",
    extracted: { basedata: { yearBuilt: 1974 } },
    extractedBy: "managed-openai",
    extractedFromRecordingIds: ["rec-1"],
    idempotencyKey: "key-1",
  });
  await db0.close();

  // Opening through the real path runs the migration. The assertion is partly
  // that this resolves at all.
  const outbox = await open(name);
  const session = await outbox.getSession("s-v0");

  expect(session).toBeDefined();
  // Nothing was transformed: the pre-existing state is intact...
  expect(session?.status).toBe("awaiting-review");
  expect(session?.extracted).toEqual({ basedata: { yearBuilt: 1974 } });
  expect(session?.extractedBy).toBe("managed-openai");
  expect(session?.ref).toBe("job-7");
  // ...and the new field is absent, which is the correct state for a session
  // extracted before usage was carried. Not `{}` — absent.
  expect(session?.extractionUsage).toBeUndefined();
  await outbox.close();
});
