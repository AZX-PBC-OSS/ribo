...
- [ ] **Step 1: Verify the problem before solving it.** Write a trivial browser test asserting `LanguageModel` exists, run it under the current config, and confirm it **skips**. Do not skip this step — if Playwright's Chromium has gained the API since the spec was measured, the rest of this task is unnecessary and should be abandoned rather than built.
- [ ] **Step 2: Choose a route** based on what Step 1 showed, and record the choice and its cost in the doc note. A manual harness is an acceptable answer; an automated path that silently never runs is not.
- [ ] **Step 3: Implement** the config change, including a per-test timeout override sufficient for a 50s call.
- [ ] **Step 4: Prove it.** The same trivial test now **runs and passes** against real Chrome. That transition — skip to pass — is the deliverable.
- [ ] **Step 5: Commit.**
