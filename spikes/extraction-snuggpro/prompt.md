# Extraction prompt — Snugg Pro bounded field set

This is the **exact prompt** the extraction step uses. It is self-contained: hand a model this
file, `schema.ts`, and one transcript, and it has everything it needs. No other context, no
retrieved documents, and — critically — **never** the ground truth.

Everything from `## System` down is the literal prompt text. Everything above it is commentary for
humans and must not be sent to the model.

This prompt carries the **normalization intent** a raw schema omits. The schema says a field is an
enum; only the prompt says how a spoken phrase becomes the right enum member, how a fused mention
splits across two axes, and how a "clean" result on a named test becomes `passed` rather than being
dropped. Doc 12's finding is that "just pass the schema" is materially wrong precisely because this
intent lives nowhere in the schema. It lives here.

> **Note on `confidence`** — the prompt asks for a confidence number, and the scoring in
> [README.md](README.md) treats it as **uncalibrated**. Whether that number is worth anything is one
> of the things this spike measures. Do not build review-card thresholds on it until the spike says
> it separates correct answers from wrong ones.
